// this code will handle the logic for our GUI
...
public class MorseCodeController {
    ...
    // hashmap is data struccture that stores the key/value pairs
    // this way , we can easily look up the morse code for any given letter as the key
    ...
    // morseMessage - contain the morse messgae after being translated
    public void playSound(String[] morseMessage) throws LineUnavailableException,InterruptedException{
       ...
       AudioFormat audioFormat = new AudioFormat(44100, 16, 1, true, false);
          ...
          //  create the dataline (to play incoming audio data)
          DataLine.Info dataLineInfo = new DataLine.Info(SourceDataLine.class, audioFormat);
          SourceDataLine sourceDataLine = (SourceDataLine) AudioSystem.getLine(dataLineInfo);
          sourceDataLine.open(audioFormat);
          sourceDataLine.start();
          
        //   duration of the sound to be played (I just messed around with the valuesbto get it cloose enough)
          int dotDuration = 200;
    ...
    private void playBeep(SourceDataLine line, int duration){
        ...
        for(int i= 0; i<data.length; i++){
            // claculate the angle of the sine wave for the  current sample based on the sample rate and frequency
            double angle = i/(44100.0/440) * 2.0 * Math.PI;
            
            // calculates the amplitudes of the sine wave at the current angle and scale it to fit within the range of
            // asigned byte (-128,127)  
            // also in the context of audio procesing, a signed bytes is often uesd to represent audio data because it
